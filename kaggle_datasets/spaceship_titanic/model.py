...
def encode(df: pd.DataFrame, string_cols) -> pd.DataFrame:
...
def extract_interesting(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # FIXME: this is calculated very wrong, when number is 2 digits, then side is calculated wrong
    df["Deck"] = df.apply(
        lambda row: "Unknown" if row["Cabin"] == "Unknown" else row["Cabin"][0], axis=1
    )
    df["Side"] = df.apply(
        lambda row: "Unknown"
        if row["Cabin"] == "Unknown"
        else row["Cabin"].rsplit("/", 1)[-1],
        axis=1,
    )

    df["Number"] = df.apply(
        lambda row: -1 if row["Cabin"] == "Unknown" else int(row["Cabin"][2]), axis=1
    )
...
def split_df(df, percentage):
    ...
    traindf = df.sample(frac=percentage, random_state=random_state)
...
def random_forest_test(df: pd.DataFrame, split: bool = True) -> RandomForestClassifier:
    if split:
        ...
        training_df = df
    model = RandomForestClassifier(n_estimators=50, random_state=42, n_jobs=8)
...
def xgboost_test(df: pd.DataFrame, test=0.0, eval=True) -> XGBClassifier:
    if eval:
        training_df, verification_df, test_df = split_into_three(df, 0.28, test)
    else:
        test_df, training_df = split_df(df, test)
    cat_cols = ["Deck", "Group"]
    for col in cat_cols:
    ...
    feature_cols = [
        col for col in df.columns if col not in ["Transported", "PassengerId"]
    ]
    if eval:
        model.fit(
    ...
    if test != 0:
        test_df["Predict"] = model.predict(test_df[feature_cols])
        assessment(test_df)
    return model
...
combined = one_hot(combined, "Side").drop("Side", axis=1)
combined = one_hot(combined, "HomePlanet").drop("HomePlanet", axis=1)
combined = one_hot(combined, "Destination").drop("Destination", axis=1)

string_cols = ["Deck", "Name", "Group", "Cabin"]
...
model = xgboost_test(df, 0.0, False)
feature_cols = [col for col in df.columns if col not in ["Transported", "PassengerId"]]

taskdf["Transported"] = model.predict(taskdf[feature_cols])
